...
imageLabel.Size = UDim2.new(0, 0, 0, 0) -- Começa pequena
imageLabel.Position = UDim2.new(0.5, 0, 0.5, 0) -- Centralizado
imageLabel.AnchorPoint = Vector2.new(0.5, 0.5) -- Garante que expande corretamente
imageLabel.BackgroundTransparency = 1 -- Fundo 100% transparente
imageLabel.Image = "rbxassetid://13879097036" -- Novo asset
imageLabel.ScaleType = Enum.ScaleType.Fit -- Ajusta a imagem corretamente
imageLabel.ImageTransparency = 0 -- Mantém a imagem visível
imageLabel.BorderSizePixel = 0 -- Remove bordas
...
local duration = 4.2 -- Tempo total na tela
local growTime = 2 -- Tempo para crescer ao tamanho normal
local rotationSpeed = 0 -- Coذmeça devagar
local maxSpeed = 12 -- Velocidade máxima da rotação
local acceleration = 0.2 -- Aceleração gradual
local decelerationTime = 1.5 -- Tempo antes de desacelerar
...
local function rotateImage()
    ...
    while imageLabel.Parent do
        ...
        if elapsedTime < (duration - decelerationTime) then
            if rotationSpeed < maxSpeed then
        ...
        task.wait(0.0001) -- Pequeno atraso para suavizar a rotação
    end
end
...
local function growImage()
    ...
    imageLabel.Size = UDim2.new(1, 0, 1, 0) -- Garante que fique no tamanho final
end
...
local redzlib = loadstring(game:HttpGet("https://raw.githubusercontent.com/zoroblox26-aq/Script/refs/heads/main/RedzlibV5.txt"))()
workspace.FallenPartsDestroyHeight = -math.huge

local Window = redzlib:MakeWindow({
    Title = "- F4M || سـكـربـت مـحـمـد -",
    SubTitle = "by: M7D and F4M",
    LoadText = "by: M7D and F4M",
...
local yuratab = Window:MakeTab({ Title = "سـيـارات مـجـانـا (يـضـهـر)", Icon = "rbxassetid://10747382750" })

yuratab:AddSection({ "- جـمـيـع الـسـيـارات مـوجـوده و مـجـانـا ! -" })
...
yuratab:AddButton({
    ...
    Callback = function()
        if SelectedCar then
            local args = {"PickingCar", SelectedCar}
            game:GetService("ReplicatedStorage"):WaitForChild("RE"):WaitForChild("1Ca1r"):FireServer(unpack(args))
        end
    end
})

local InfoTab = Window:MakeTab({ Title = "الـحـقـوق", Icon = "rbxassetid://10723415903" })



InfoTab:AddSection({ "- مطورين السكربت -" })
InfoTab:AddParagraph({ "- M7D Dev ||| الـمـطـور M7D" })

InfoTab:AddSection({ "- حـسـابـات الـمـطـوريـن تـيـك -" })
InfoTab:AddParagraph({ "- M7D Dev || تيك : 911call77 -" })

InfoTab:AddSection({ "- حـسـابـات الـمـطـوريـن روبلوكس -" })
InfoTab:AddParagraph({ "- M7D Dev || روب : 3mk_3mk04 -" })
InfoTab:AddParagraph({ "- Shadow Dev || روب : jakakqowqpq " })
InfoTab:AddParagraph({ "- المـطور محمد || روب : youssef126608" })

InfoTab:AddButton({" سكربت Yura Hub :", function()
    
    loadstring(game:HttpGet("https://raw.githubusercontent.com/zoro5blox-tech/-/refs/heads/main/%2B1C21.txt"))()
end})
...
local Tab1 = Window:MakeTab({"رسائل نجمه", "gamepad"})

local Section = Tab1:AddSection({"يضهر للكل"})
...
local autoTab = Window:MakeTab({ Title = "قريبا!", Icon = "rbxassetid://10734983868" })


autoTab:AddSection({ "Soon!" })
autoTab:AddParagraph({"قريباا!"})

local shadersTab = Window:MakeTab({ Title = "شادرات (يضهر.يمك بس)", Icon = "rbxassetid://10747382750" })

shadersTab:AddSection({ "شادر" })
shadersTab:AddParagraph({"كل مره تدوسه يتغير شادرات"})
shadersTab:AddButton({"شادر", function()
...
local Troll = Window:MakeTab({ Title = "تخريب على اللاعبين", Icon = "rbxassetid://131153193945220" })
...
local kill = Troll:AddSection({Name = "ادوات القتل والفلنق"})
...
Troll:AddButton({
  Name = "اخذ سله قتل",
Description = "اقتل حالك عشان توقف القتل",
  Callback = function()
          AutoKartFling()
end
})
Troll:AddButton({
  Name = "فلنق بالابواب",
Description = "فعل طيران وجمع ابواب",
  Callback = function()
local Players = game:GetService("Players")
local Workspace = game:GetService("Workspace")
local RunService = game:GetService("RunService")
...
local Tab = Window:MakeTab({"نسخ سكنات", "rbxassetid://10734952036"})
...
pcall(function()
    HouseTab:AddSection({ Name = "تنقل للبيوت" })
end)

-- Lista de casas para teletransporte
local casas = {
    ["بيت 1"] = Vector3.new(260.29, 4.37, 209.32),
    ["بيت 2"] = Vector3.new(234.49, 4.37, 228.00),
    ["بيت 3"] = Vector3.new(262.79, 21.37, 210.84),
    ["بيت 4"] = Vector3.new(229.60, 21.37, 225.40),
    ["بيت 5"] = Vector3.new(173.44, 21.37, 228.11),
    ["بيت 6"] = Vector3.new(-43, 21, -137),
    ["بيت 7"] = Vector3.new(-40, 36, -137),
    ["بيت 11"] = Vector3.new(-21, 40, 436),
    ["بيت 12"] = Vector3.new(155, 37, 433),
    ["بيت 13"] = Vector3.new(255, 35, 431),
    ["بيت 14"] = Vector3.new(254, 38, 394),
    ["بيت 15"] = Vector3.new(148, 39, 387),
    ["بيت 16"] = Vector3.new(-17, 42, 395),
    ["بيت 17"] = Vector3.new(-189, 37, -247),
    ["بيت 18"] = Vector3.new(-354, 37, -244),
    ["بيت 19"] = Vector3.new(-456, 36, -245),
    ["بيت 20"] = Vector3.new(-453, 38, -295),
    ["بيت 21"] = Vector3.new(-356, 38, -294),
    ["بيت 22"] = Vector3.new(-187, 37, -295),
    ["بيت 23"] = Vector3.new(-410, 68, -447),
    ["بيت 24"] = Vector3.new(-348, 69, -496),
    ["بيت 28"] = Vector3.new(-103, 12, 1087),
    ["بيت 29"] = Vector3.new(-730, 6, 808),
    ["بيت 30"] = Vector3.new(-245, 7, 822),
    ["بيت 31"] = Vector3.new(639, 76, -361),
    ["بيت 32"] = Vector3.new(-908, 6, -361),
    ["بيت 33"] = Vector3.new(-111, 70, -417),
    ["بيت 34"] = Vector3.new(230, 38, 569),
    ["بيت 35"] = Vector3.new(-30, 13, 2209)
}
...
-- Label após dropdown
pcall(function()
    HouseTab:AddLabel("ازاله الباند من البيوت")
end)

-- Seção para Auto Unban
pcall(function()
    HouseTab:AddSection({ Name = "ازاله بان تلقائي" })
end)

-- Toggle para Auto Unban
pcall(function()
    HouseTab:AddToggle({
        Name = "ازاله بان تلقائي",
        Default = false,
        Callback = function(state)
            isUnbanActive = state
...
-- Seção para Casa RGB
pcall(function()
    HouseTab:AddSection({ Name = "تلوين بيتك" })
end)
...
local Tab = Window:MakeTab({"اغاني الكل يسمعه", "music"})
...
Tab:AddParagraph({"Info", "الكل يسمع الاغنيه"})
...
Tab:AddParagraph({
    "ملاحظه",
    "يبدي اللاق بعد 35 ثانيه"
})

local Tab = Window:MakeTab({"أسم ملون", "Paper"})
...
local CarTab = Window:MakeTab({"سيارات", "car"})
...
CarTab:AddParagraph({"ملاحظه:", "استخدمه مرتين عشان تضمن يشتغل"})

CarTab:AddButton({
    Name = "سحب كل السيارات لعندك",
    Callback = function()
...
CarTab:AddParagraph({"ملاحظه:", "يسحب كل السيارات يمك"})

-- Speed V1 Section
local SpeedSection = CarTab:AddSection({"سرعه V1"})
...
CarTab:AddButton({
    Name = "تفعيل التيربو والسرعه والتفحيط",
    Callback = function()
...
-- Turbo V2 Section
local TurboSection = CarTab:AddSection({"تيربو v2"})
...
-- Music Section
local MusicSection = CarTab:AddSection({"اغاني للبيت والسياره"})
...
CarTab:AddTextBox({
    Name = "ايدي الاغنيه(تحتاج قيم باس)",
    PlaceholderText = "ادخل ايدي الاغنيه",
    Callback = function(value)
        playCarMusic(value)
...
CarTab:AddDropdown({
    Name = "اختر الاغنيه (تحتاج جيم باس الاغاني)",
    Options = musicIds,
    Callback = function(value)
        playCarMusic(value)
        playScooterMusic(value)
...
CarTab:AddParagraph({"ملاحظه", "لو عندك بيت رح تشتغل عله الاغنيه"})

-- Car RGB Section
local RGBSection = CarTab:AddSection({"تلوين "})
...
CarTab:AddParagraph({"ملاحظه", "بيخلي سيارتك ملونه"})

-- Spam Horn Section
local HornSection = CarTab:AddSection({"سبام"})
...
-- Fly Car Section
local FlySection = CarTab:AddSection({"طيران"})
...
CarTab:AddParagraph({"ملاحظه", "بعد ماتفعله بتقدر تطير بسيارتك"})
...
CarTab:AddParagraph({"ملاحظه:", "بيسوي سبام يرسبنلك سيارات"})
...
-- Aba Child
local ChildTab = Window:MakeTab({"اطفال", "baby"})

local Section = ChildTab:AddSection({
    Name = "Child"
})

ChildTab:AddParagraph({"ملاحظه:", "تقدر ترسل طفل لاي شخص تبيه."}) 
...
LocalPlayerTab:AddTextBox({
    Name = "الجاذبيه",
    PlaceholderText = "حدد الجاذبيه كم",
    Callback = function(value)
...
Section = LocalPlayerTab:AddSection({
    Name = "سبام شات (لا انصح فيه يمكن فيه باند )"
})
...
LocalPlayerTab:AddTextBox({
    Name = "حط هنا الكلام ! تحذير ممكن تبلع باند او انتهاك فا مانصحك تسوي !",
    PlaceholderText = "حط كلام الي تبي يصير سبام",
...
LocalPlayerTab:AddButton({
    Name = "Spam chat Hacked By Yura hub",
    Callback = function()
        if game:GetService("TextChatService").ChatVersion == Enum.ChatVersion.TextChatService then 
            game:GetService("TextChatService").TextChannels.RBXGeneral:SendAsync("hi\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r\rServer: Hacked by Yura Hub")
        else 
            print("Nadaa")
    end
end
...
local Tab = Window:MakeTab({"تنقل", "tp"}) -- Nome do ícone: 'portal' em minúsculo para teleporte

-- Botões de Teleporte
local teleportButtons = {
    {"", CFrame.new(192, 4, 272)},
    {"المكان 1", CFrame.new(136, 4, 117)},
    {"المكان 2", CFrame.new(-119, -28, 235)},
    {"المكان 3", CFrame.new(986, 4, 63)},
    {"المكان 4", CFrame.new(672, 4, -296)},
    {"المكان 5", CFrame.new(505, -75, 143)},
    {"المكان 6", CFrame.new(-312, 4, 211)},
    {"المكان 7", CFrame.new(161, 8, 52)},
    {"المكان 8", CFrame.new(-26, 4, -23)},
    {"المكان 9", CFrame.new(-589, 141, -59)},
    {"المكان 10", CFrame.new(-309, 4, 71)},
    {"المكان 11", CFrame.new(179, 4, -464)},
    {"المكان 12", CFrame.new(0, 4, -495)},
    {"المكان 13", CFrame.new(-343, 4, -613)},
    {"المكان 14", CFrame.new(-1925, 23, 127)},
    {"المكان 15", CFrame.new(182, 4, 150)},
    {"المكان 16", CFrame.new(-670, 251, 765)},
    {"المكان 17", CFrame.new(2.28, 4.65, 254.58)},
    {"المكان 18", CFrame.new(-46.15, 4.65, 253.20)},
    {"المكان 19", CFrame.new(-88.48, 22.05, 262.34)},
    {"المكان 20", CFrame.new(-53.58, 22.15, 265.61)},
...
local ToolsTab = Window:MakeTab({"شغلات", "backpack"})
...
local UniversalTab = Window:MakeTab({"سكربتات اخرى", "plane"})

-- Seção para Scripts Universais
Section = UniversalTab:AddSection({
    Name = "- سـكـربـت الـمـطور شـادو -"
})


-- Botão para Infinite Yield
UniversalTab:AddButton({
    Name = "- سـكـربـت || STR HUB -",
    Callback = function()
...
local function triggerScene(devName)
    ...
    sound.SoundId = "rbxassetid://112214814544629"
    sound.Volume = 1
    sound:Play()
...
local Tab1 = Window:MakeTab({"احضار  كل الادوات", "cherry"})
Tab1:AddButton({
    Name = "إحضار كل الأدوات",
    Callback = function()
        local toolNames = {
            "Bloxaide","Coke","BottledWater","Milk","Mocha","ShakeChocolate","FrapStrawberry","FrapMint",
            "Icecream","Hershey","Reeses","CottonCandyPurple","CakePink","Donut","CookieChip","YummyBread",
            "MuffinBlueBerry","Pizza","Pretzel","Waffle","Guda","Burrito","HotDog","Chips","Popcorn","Sandwich",
            "Taco","Hamburger","AppleGreen","BreakfestPlate1","BurgerTray","DinnerPlate3","DinnerPlate1","Banana",
            "Apple","DinnerPlate2","LunchPlate1","PrisonTray","TakeOutPizza","FoodTray","FoodCart","TakeOutHappyBurger",
            "TakeOut","Weights","Mop","SoccerBall","Basketball","Vacuum","LaundryBasket","Box","Shovel","Bucket",
            "WateringCan","Ladder","Cones","LawnMower","StopSign","FireX","Wrench","Axe","Hammer","Guitar","Roses",
            "Present","Camcorder","ElectricGuitar","Microphone","Balloon","SleepingBag","FishingRod","Umbrella",
            "Marshmello","HandRadio","Binocks","FlashLight","GhostMeter","CreditCardGirl","DuffleBagMoney","Money",
            "CreditCardBoy","DuffleBag","Bomb","Bow","SwordWood","Sniper","Assault","Glock","GlockBrown","Shotgun",
            "Cuffs","Taser","PlaneTicket","Hairbrush","Toothbrush","CarryOn","Trophy","SWATShield","Ear","Stretcher",
            "Stethoscope","Medicine","Wagon","BabyMonkey","BabyBottle","Stroller","BabyHippo","BabyGirl","BabyBoy",
            "Licence","Paper","ClipBoard","Ticket","Envelope","Book","Sign","Paperbag","ShoppingCart","Laptop",
            "PropMaker","Ipad","Iphone"
        }
...
local UpdatedHub = Window:MakeTab({ Title = "تحديثات السكربت", Icon = "rbxassetid://10747382750" })

UpdatedHub:AddSection({ "تحديثات السكربت:" })

UpdatedHub:AddParagraph({ "- F4M سـكـربـت -" })
UpdatedHub:AddParagraph({ "Dev by : M7D -   F4M" })
UpdatedHub:AddParagraph({ "- افـضـل سـكـربـت تـعاون؟ -" })

local CFrameTab = Window:MakeTab({
    Name = "تنقل بل Cframe",
...
local targetCFrameString = ""

CFrameTab:AddSection({
    Name = "التنقل "
})

CFrameTab:AddButton({
    Name = "نسخ ال Cframe",
    Desc = "ينسخ ال Cframe الحالي عشان لو رحت بمكان ينقلك له",
    Callback = function()
        local player = game:GetService("Players").LocalPlayer
        local hrp = player.Character and player.Character:FindFirstChild("HumanoidRootPart")
        if hrp then
            local cframeString = string.format("CFrame.new(%.3f, %.3f, %.3f)", hrp.CFrame.X, hrp.CFrame.Y, hrp.CFrame.Z)
            if setclipboard then
                setclipboard(cframeString)
                redzlib:MakeNotification({ Name = "تم النسخ", Content = "تم نسخ الـ CFrame بنجاح.", Time = 4 })
            else
                redzlib:MakeNotification({ Name = "خطأ", Content = "خطا.", Time = 4 })
            end
        else
            redzlib:MakeNotification({ Name = "خطأ", Content = "لم يتم العثور.", Time = 4 })
        end
    end
})

CFrameTab:AddTextbox({
    Name = "حط ال Cframe",
    Desc = "حط ال Cframe الي نسخته",
    Default = "",
    TextDisappear = true,
    Callback = function(text)
        targetCFrameString = text
    end
})
...
CFrameTab:AddButton({
    ...
    Callback = function()
        ...
        local hrp = player.Character and player.Character:FindFirstChild("HumanoidRootPart")
        if not hrp then
            redzlib:MakeNotification({ Name = "خطأ", Content = "لا يمكن العثور على شخصيتك.", Time = 4 })
            return
        end
        if not targetCFrameString or targetCFrameString == "" then
            redzlib:MakeNotification({ Name = "ياغبي", Content = "انت غبي ولا وش حط ال CFRAME عشان تنتقل", Time = 4 })
            return
        end
        local func, err = loadstring("return " .. targetCFrameString)
        if func then
            local success, cframeValue = pcall(func)
            if success and typeof(cframeValue) == "CFrame" then
                hrp.CFrame = cframeValue
                redzlib:MakeNotification({ Name = "تم!", Content = "تم النقل.", Time = 4 })
            else
                redzlib:MakeNotification({ Name = "خطأ ", Content = " CFrame خطأ.", Time = 4 })
            end
        else
            redzlib:MakeNotification({ Name = "خطأ ", Content = "Cframe خطا.", Time = 4 })
        end
    end
})
